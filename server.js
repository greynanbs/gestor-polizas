...
const C = {
  ...
  cuotasMensuales:() => db.collection('cuotas_mensuales'),
};
...
async function calcComision(asegNombre, ramoNombre, vendNombre, primaNeta, tipoContrato) {
  const [aseg, vend] = await Promise.all([
    C.aseguradoras().findOne({nombre:asegNombre}),
    C.vendedores().findOne({nombre:vendNombre})
  ]);
  const tc = tipoContrato || 'nuevo';
  const porTipo = aseg?.comisionesPorTipo?.[ramoNombre];
  const pA = porTipo ? (porTipo[tc] || 0) : ((aseg?.comisiones?.[ramoNombre]) || 0);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    // HEALTH CHECK — UptimeRobot usa HEAD
    if (method==='HEAD'&&(pn==='/'||pn==='/index.html')) {
    ...
    if (pn==='/api/polizas') {
      ...
      if (method==='POST') {
        ...
        if (d.aseguradora&&d.tipo&&d.vendedor&&d.primaNeta) {
          d.comision=await calcComision(d.aseguradora,d.tipo,d.vendedor,parseFloat(d.primaNeta)||0,d.tipoContrato);
        }
    ...
    const simples = {
      ...
      'cuotas-mensuales':  C.cuotasMensuales,
    };
