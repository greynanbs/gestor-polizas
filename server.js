...
async function seed() {
  // Upsert: crea o actualiza el admin siempre con las credenciales correctas
  await C.usuarios().updateOne(
    { usuario: "greyna" },
    {
      $set: {
        usuario: "greyna",
  ...
  console.log("Usuario administrador verificado/actualizado correctamente.");
}
...
const app = async (req, res) => {
  ...
  try {
    ...
    if (pn === '/api/login' && method === 'POST') {
      ...
      // El frontend envía el campo como "clave"; también soportamos "password" por compatibilidad
      const claveRecibida = d.clave || d.password || '';
      ...
      if (!u || u.password !== claveRecibida) { fail('Usuario o contraseña incorrectos', 401); return; }
      ok({ ok: true, usuario: { usuario: u.usuario, nombre: u.nombre, rol: u.rol }, nombre: u.nombre, rol: u.rol });
      return;
    }
    ...
    if (pn === '/api/reset-admin' && method === 'GET') {
      await C.usuarios().updateOne(
        { usuario: "greyna" },
        { $set: { usuario: "greyna", email: "admin@reynaseguros.com", password: "Keanu2018", rol: "administrador", nombre: "Gerente Reyna" } },
        { upsert: true }
      );
      ok({ ok: true, mensaje: "Usuario admin restaurado correctamente." }); return;
    }
