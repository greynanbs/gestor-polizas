...
function renderAdminAseguradoras(c) {
  ...
  c.innerHTML=`<div style="display:flex;justify-content:flex-end;margin-bottom:10px"><button class="btn btn-primary btn-sm" onclick="abrirModalAseguradora()"><i class="ti ti-plus" aria-hidden="true"></i> Nueva aseguradora</button></div><div class="card-list">${lista.map(a=>{
    const rConPct=ramos.filter(r=>{
      const pt=a.comisionesPorTipo?.[r.nombre];
      if(pt)return(pt.nuevo>0||pt.renovacion>0||pt.agenciamiento>0);
      return(a.comisiones&&a.comisiones[r.nombre]>0);
    });
    const subtitulo=rConPct.length?rConPct.map(r=>{
      const pt=a.comisionesPorTipo?.[r.nombre];
      if(pt)return`${r.nombre}: ${pt.nuevo||0}/${pt.renovacion||0}/${pt.agenciamiento||0}%`;
      return`${r.nombre}: ${a.comisiones[r.nombre]}%`;
    }).join(' · '):'Sin comisiones configuradas';
    return`<div class="list-card"><div><div class="list-card-title">${a.nombre}</div><div class="list-card-sub">${subtitulo}</div></div><div class="row-actions"><button class="btn-icon" onclick="abrirModalAseguradora('${a.id}')" title="Editar"><i class="ti ti-edit"></i></button>${canDel?`<button class="btn-icon danger" onclick="eliminarAseguradora('${a.id}')" title="Eliminar"><i class="ti ti-trash"></i></button>`:''}</div></div>`;
  }).join('')||'<div class="empty-state"><i class="ti ti-building"></i>No hay aseguradoras.</div>'}</div>`;
}
function abrirModalAseguradora(id=null) {
  ...
  document.getElementById('aseg-pct-body').innerHTML=rO.map(r=>{
    const pctViejo=(aseg?.comisiones?.[r.nombre])||0;
    const porTipo=aseg?.comisionesPorTipo?.[r.nombre];
    const nuevo=porTipo?.nuevo ?? pctViejo;
    const renov=porTipo?.renovacion ?? pctViejo;
    const agen=porTipo?.agenciamiento ?? pctViejo;
    return `<tr><td>${r.nombre}</td>`
      +`<td><input type="number" step="0.01" min="0" max="100" data-ramo="${r.nombre}" data-tipo="nuevo" class="aseg-pct-input" value="${nuevo}"></td>`
      +`<td><input type="number" step="0.01" min="0" max="100" data-ramo="${r.nombre}" data-tipo="renovacion" class="aseg-pct-input" value="${renov}"></td>`
      +`<td><input type="number" step="0.01" min="0" max="100" data-ramo="${r.nombre}" data-tipo="agenciamiento" class="aseg-pct-input" value="${agen}"></td>`
      +`</tr>`;
  }).join('');
...
async function guardarAseguradora() {
  ...
  document.querySelectorAll('.aseg-pct-input').forEach(i=>{
    const ramo=i.dataset.ramo;
    const tipo=i.dataset.tipo;
    const val=parseFloat(i.value)||0;
    if(!comisionesPorTipo[ramo])comisionesPorTipo[ramo]={nuevo:0,renovacion:0,agenciamiento:0};
    comisionesPorTipo[ramo][tipo]=val;
    if(tipo==='nuevo')comisiones[ramo]=val;
  });
  try {
    if(editingId){const a=await apiPut('aseguradoras',editingId,{nombre,comisiones,comisionesPorTipo});const i=aseguradoras.findIndex(x=>x.id===editingId);aseguradoras[i]=a;mostrarToast('Aseguradora actualizada');}
    else{const nv=await apiPost('aseguradoras',{nombre,comisiones,comisionesPorTipo});aseguradoras.push(nv);mostrarToast('Aseguradora creada');}
