...
function renderTabla() {
  ...
  if(!lista.length){tbody.innerHTML=`<tr><td colspan="12"><div class="empty-state"><i class="ti ti-search"></i>No se encontraron pólizas.</div></td></tr>`;return;}
  tbody.innerHTML=lista.map(p=>{
    ...
    const tc=p.tipoContrato||'nuevo';
    const tcAbbr={nuevo:'N',renovacion:'R',agenciamiento:'A'}[tc]||'N';
    const tcColor={nuevo:'#16a34a',renovacion:'#2563eb',agenciamiento:'#7c3aed'}[tc]||'#64748b';
    const tcLabel={nuevo:'Nuevo',renovacion:'Renovación',agenciamiento:'Agenciamiento'}[tc]||'Nuevo';
    const tcBadge=`<span title="${tcLabel}" style="display:inline-block;min-width:22px;padding:2px 6px;border-radius:4px;background:${tcColor};color:#fff;font-size:11px;font-weight:600;text-align:center">${tcAbbr}</span>`;
    return`<tr>
      ...
      <td style="text-align:center">${tcBadge}</td>
...
function enlazarEventosComision() {
  ['f-aseguradora','f-tipo','f-vendedor','f-primaNeta','f-tipoContrato'].forEach(id=>{
...
function actualizarComisionPreview() {
  ...
  const tipoContrato=(document.getElementById('f-tipoContrato')||{}).value||'nuevo';
  ...
  const porTipo=a?.comisionesPorTipo?.[ramo];
  const pA=porTipo?(porTipo[tipoContrato]||0):((a?.comisiones?.[ramo])||0);
  const pV=(v?.comisiones?.[ramo])||0;
  const cB=(primaNeta*pA)/100;
  const cV=(cB*pV)/100;
  const etiquetaTipo=tipoContrato==='nuevo'?'contrato nuevo':(tipoContrato==='renovacion'?'renovación':'agenciamiento');
  ...
  if(esVendedorRestringido()) {
    ...
    box.innerHTML=`<i class="ti ti-calculator" style="margin-right:4px" aria-hidden="true"></i>Comisión bróker (${pA}% de prima neta, ${etiquetaTipo}): <strong>${fmtMoney(cB)}</strong> · Comisión vendedor (${pV}% de la comisión bróker): <strong>${fmtMoney(cV)}</strong>`;
  }
}

function calcularFechaCuota(inicioStr, diaAniv, mesOffset){
  const d=new Date(inicioStr+'T00:00:00');
  const targetY=d.getFullYear();
  const targetM=d.getMonth()+mesOffset;
  const ultimoDia=new Date(targetY, targetM+1, 0).getDate();
  const dia=Math.min(diaAniv, ultimoDia);
  const f=new Date(targetY, targetM, dia);
...
function generarCuotasMensuales(p){
  const cuotas=[];
  const genCobro=p.formaCobroAseg==='mensual';
  const genPago=p.formaPagoVend==='mensual';
  if(!genCobro&&!genPago)return cuotas;
  const a=aseguradoras.find(x=>x.nombre===p.aseguradora);
  const v=vendedores.find(x=>x.nombre===p.vendedor);
  const tipoC=p.tipoContrato||'nuevo';
  const porTipo=a?.comisionesPorTipo?.[p.tipo];
  const pctA=porTipo?(porTipo[tipoC]||0):((a?.comisiones?.[p.tipo])||0);
  const pctV=(v?.comisiones?.[p.tipo])||0;
  const primaMensual=(p.primaNeta||0)/12;
  const cobroMensual=primaMensual*pctA/100;
  const pagoMensual=cobroMensual*pctV/100;
  const dia=parseInt(p.diaAniversario)||1;
  for(let i=0;i<12;i++){
    const fecha=calcularFechaCuota(p.inicio,dia,i);
    const base={polizaId:p.id,polizaCliente:p.cliente,polizaNumero:p.numero||'',
      aseguradora:p.aseguradora,ramo:p.tipo,vendedor:p.vendedor,
      numeroCuota:i+1,fechaVencimiento:fecha,primaNetaMensual:primaMensual,
      estado:'pendiente',fechaLiquidada:null,liquidacionId:null};
    if(genCobro)cuotas.push({...base,tipoCuota:'cobro',pctComision:pctA,monto:cobroMensual});
    if(genPago)cuotas.push({...base,tipoCuota:'pago',pctComision:pctV,monto:pagoMensual});
  }
  return cuotas;
}

async function sincronizarCuotasMensuales(p){
  try {
    const todas=await apiGet('cuotas-mensuales');
    const deEstaPoliza=todas.filter(c=>c.polizaId===p.id);
    const pendientes=deEstaPoliza.filter(c=>c.estado==='pendiente');
    for(const c of pendientes){await apiDel('cuotas-mensuales',c.id);}
    const nuevas=generarCuotasMensuales(p);
    for(const c of nuevas){await apiPost('cuotas-mensuales',c);}
  } catch(e){console.error('Error sincronizando cuotas:',e);}
}
...
async function guardarPoliza() {
  ...
  try {
    ...
    if(editingId){
      ...
      polizas[i]=act; polizaGuardada=act; mostrarToast('Póliza actualizada');
    } else {
      const nv=await apiPost('polizas',data);
      polizas.push(nv); polizaGuardada=nv; mostrarToast('Póliza registrada');
      await cargarComisiones();
    }
    await sincronizarCuotasMensuales(polizaGuardada);
...
async function eliminarPoliza(id) {
  ...
  try{
    try{
      const todas=await apiGet('cuotas-mensuales');
      const pendientes=todas.filter(c=>c.polizaId===id&&c.estado==='pendiente');
      for(const c of pendientes){await apiDel('cuotas-mensuales',c.id);}
    }catch(e){console.error('Error borrando cuotas:',e);}
    await apiDel('polizas',id);polizas=polizas.filter(x=>x.id!==id);
    await cargarComisiones();render();mostrarToast('Póliza eliminada');
