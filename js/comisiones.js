...
function abrirModalMovPago(){
  ...
  const list=document.getElementById('mp-liq-list');
  if(list)list.innerHTML='Selecciona un vendedor primero';
...
function cargarLiquidacionesVendedor(){
  ...
  const list=document.getElementById('mp-liq-list');
  if(!list)return;
  if(!vend){list.innerHTML='Selecciona un vendedor primero';return;}
  // Liquidaciones pendientes = totalNeto > totalCobradoPrev (calculado por movimientos previos)
  const liqsVend=liquidaciones.filter(l=>l.vendedor===vend)
    .sort((a,b)=>new Date(a.fecha||a.fechaCreacion||0)-new Date(b.fecha||b.fechaCreacion||0)); // FIFO más viejas primero
  const pendientes=liqsVend.filter(l=>{
    const pagadoPrev=movPagos.filter(m=>m.tipo==='Pago liquidación' && (m.liquidacionIds||[]).includes(l.id)).reduce((s,m)=>{
      const asignado=(m.asignaciones||[]).find(a=>a.liquidacionId===l.id);
      return s+(asignado?asignado.monto:0);
    },0);
    // Retro-compatibilidad con formato viejo (liquidacionId único)
    const pagadoLegacy=movPagos.filter(m=>m.tipo==='Pago liquidación' && m.liquidacionId===l.id && !(m.liquidacionIds||[]).length).reduce((s,m)=>s+(parseFloat(m.monto)||0),0);
    return (parseFloat(l.totalNeto)||0) - pagadoPrev - pagadoLegacy > 0.001;
  });
  if(!pendientes.length){
    list.innerHTML='<div style="color:#94a3b8;padding:6px 0">No hay liquidaciones pendientes. Puedes registrar un anticipo libre.</div>';
    return;
  }
  list.innerHTML=pendientes.map(l=>{
    const fecha=new Date(l.fecha||l.fechaCreacion||Date.now()).toLocaleDateString('es-EC');
    const pagadoPrev=movPagos.filter(m=>m.tipo==='Pago liquidación' && (m.liquidacionIds||[]).includes(l.id)).reduce((s,m)=>{
      const asignado=(m.asignaciones||[]).find(a=>a.liquidacionId===l.id);
      return s+(asignado?asignado.monto:0);
    },0);
    const pagadoLegacy=movPagos.filter(m=>m.tipo==='Pago liquidación' && m.liquidacionId===l.id && !(m.liquidacionIds||[]).length).reduce((s,m)=>s+(parseFloat(m.monto)||0),0);
    const saldo=(parseFloat(l.totalNeto)||0)-pagadoPrev-pagadoLegacy;
    return `<label style="display:flex;align-items:center;gap:8px;padding:4px 0;color:#1a2744;cursor:pointer">
      <input type="checkbox" class="mp-liq-check" data-id="${l.id}" data-saldo="${saldo.toFixed(2)}" onchange="onMovPagoLiqChange()">
      <span>#LIQ — ${fecha} — Total ${fmtMoney(l.totalNeto)} · <strong style="color:#dc2626">Saldo: ${fmtMoney(saldo)}</strong></span>
    </label>`;
  }).join('');
}

function onMovPagoLiqChange(){
  const checks=document.querySelectorAll('.mp-liq-check:checked');
  let sumaSaldos=0; checks.forEach(ch=>sumaSaldos+=parseFloat(ch.dataset.saldo)||0);
  const inp=document.getElementById('mp-monto');
  if(inp && sumaSaldos>0) inp.value=sumaSaldos.toFixed(2);
}
...
async function guardarMovPago(){
  ...
  const checksLiq=document.querySelectorAll('.mp-liq-check:checked');
  const liquidacionIds=[...checksLiq].map(c=>c.dataset.id);
  ...
  // Aplicar monto en orden FIFO (más viejas primero)
  const asignaciones=[];
  if(liquidacionIds.length){
    const liqsSeleccionadas=liquidaciones.filter(l=>liquidacionIds.includes(l.id))
      .sort((a,b)=>new Date(a.fecha||a.fechaCreacion||0)-new Date(b.fecha||b.fechaCreacion||0));
    let restante=monto;
    for(const l of liqsSeleccionadas){
      const chk=[...checksLiq].find(c=>c.dataset.id===l.id);
      const saldo=chk?parseFloat(chk.dataset.saldo)||0:0;
      if(restante<=0)break;
      const aplicar=Math.min(restante,saldo);
      asignaciones.push({liquidacionId:l.id,monto:Math.round(aplicar*100)/100});
      restante-=aplicar;
    }
    // Si sobra monto, se registra como sobrante (queda como anticipo implícito)
  }
  try{
    const payload={vendedor,tipo,monto,fecha,comprobante,descripcion,
      liquidacionIds, asignaciones,
      // Retro-compat: si sola una liquidación, mantener liquidacionId único
      liquidacionId: liquidacionIds.length===1?liquidacionIds[0]:null,
      aplicado:false};
    const nv=await apiPost('movimientos-pagos',payload);
    movPagos.unshift(nv);cerrarModal('overlay-movpago');renderContabilidad();
    mostrarToast(liquidacionIds.length>1?`Pago aplicado a ${liquidacionIds.length} liquidaciones (FIFO)`:'Movimiento registrado');
...
function abrirModalMovCobro(){
  ...
  const list=document.getElementById('mc-liq-list');
  if(list)list.innerHTML='Selecciona una aseguradora primero';
...
function cargarLiqCobrosAseguradora(){
  ...
  const list=document.getElementById('mc-liq-list');
  if(!list)return;
  if(!aseg){list.innerHTML='Selecciona una aseguradora primero';return;}
  const liqsAseg=liqCobros.filter(l=>l.aseguradora===aseg)
    .sort((a,b)=>new Date(a.fecha||a.fechaCreacion||0)-new Date(b.fecha||b.fechaCreacion||0));
  const pendientes=liqsAseg.filter(l=>{
    const cobradoPrev=movCobros.filter(m=>m.tipo==='Cobro liquidación' && (m.liquidacionCobroIds||[]).includes(l.id)).reduce((s,m)=>{
      const asignado=(m.asignaciones||[]).find(a=>a.liquidacionCobroId===l.id);
      return s+(asignado?asignado.monto:0);
    },0);
    const cobradoLegacy=movCobros.filter(m=>m.tipo==='Cobro liquidación' && m.liquidacionCobroId===l.id && !(m.liquidacionCobroIds||[]).length).reduce((s,m)=>s+(parseFloat(m.monto)||0),0);
    return (parseFloat(l.totalNeto)||0) - cobradoPrev - cobradoLegacy > 0.001;
  });
  if(!pendientes.length){
    list.innerHTML='<div style="color:#94a3b8;padding:6px 0">No hay liquidaciones pendientes.</div>';
    return;
  }
  list.innerHTML=pendientes.map(l=>{
    const fecha=new Date(l.fecha||l.fechaCreacion||Date.now()).toLocaleDateString('es-EC');
    ...
    const cobradoLegacy=movCobros.filter(m=>m.tipo==='Cobro liquidación' && m.liquidacionCobroId===l.id && !(m.liquidacionCobroIds||[]).length).reduce((s,m)=>s+(parseFloat(m.monto)||0),0);
    const saldo=(parseFloat(l.totalNeto)||0)-cobradoPrev-cobradoLegacy;
    return `<label style="display:flex;align-items:center;gap:8px;padding:4px 0;color:#1a2744;cursor:pointer">
      <input type="checkbox" class="mc-liq-check" data-id="${l.id}" data-saldo="${saldo.toFixed(2)}" onchange="onMovCobroLiqChange()">
      <span>${aseg} — ${fecha} — Total ${fmtMoney(l.totalNeto)} · <strong style="color:#dc2626">Saldo: ${fmtMoney(saldo)}</strong></span>
    </label>`;
  }).join('');
}

function onMovCobroLiqChange(){
  const checks=document.querySelectorAll('.mc-liq-check:checked');
  let sumaSaldos=0; checks.forEach(ch=>sumaSaldos+=parseFloat(ch.dataset.saldo)||0);
  const inp=document.getElementById('mc-monto');
  if(inp && sumaSaldos>0) inp.value=sumaSaldos.toFixed(2);
}
...
async function guardarMovCobro(){
  ...
  const checksLiq=document.querySelectorAll('.mc-liq-check:checked');
  const liquidacionCobroIds=[...checksLiq].map(c=>c.dataset.id);
  ...
  const asignaciones=[];
  if(liquidacionCobroIds.length){
    const liqsSel=liqCobros.filter(l=>liquidacionCobroIds.includes(l.id))
      .sort((a,b)=>new Date(a.fecha||a.fechaCreacion||0)-new Date(b.fecha||b.fechaCreacion||0));
    let restante=monto;
    for(const l of liqsSel){
      const chk=[...checksLiq].find(c=>c.dataset.id===l.id);
      const saldo=chk?parseFloat(chk.dataset.saldo)||0:0;
      if(restante<=0)break;
      const aplicar=Math.min(restante,saldo);
      asignaciones.push({liquidacionCobroId:l.id,monto:Math.round(aplicar*100)/100});
      restante-=aplicar;
    }
  }
  try{
    const payload={aseguradora,tipo,monto,fecha,comprobante,descripcion,
      liquidacionCobroIds, asignaciones,
      liquidacionCobroId: liquidacionCobroIds.length===1?liquidacionCobroIds[0]:null};
    const nv=await apiPost('movimientos-cobros',payload);
    movCobros.unshift(nv);
    await cargarPolizas();await cargarLiqCobros();
    cerrarModal('overlay-movcobro');renderContabilidad();
    mostrarToast(liquidacionCobroIds.length>1?`Cobro aplicado a ${liquidacionCobroIds.length} liquidaciones (FIFO)`:'Cobro registrado');
  }catch(e){err.textContent='Error al guardar';err.style.display='block';}
}
