...
function cargarComisionesParaLiquidar(){
  ...
  const pendientes=comisiones.filter(cm=>cm.vendedor===vend&&cm.facturado&&!cm.pagado);
  if(!pendientes.length){tbody.innerHTML='<tr><td colspan="7"><div class="empty-state" style="padding:1rem">No hay comisiones facturadas pendientes de pago.</div></td></tr>';document.getElementById('liq-resumen').innerHTML='';return;}
  const ivaPct=parseFloat(document.getElementById('liq-iva-pct').value)||15;
  tbody.innerHTML=pendientes.map(cm=>`<tr>
    <td><input type="checkbox" class="liq-com-check" data-id="${cm.id}" data-monto="${cm.comisionVendedor}" checked onchange="recalcularLiquidacion()"></td>
    <td>${fd(cm.inicio||cm.fechaCreacion)}</td>
    <td>${cm.cliente}</td>
    <td>${cm.ramo}</td>
    <td>${cm.aseguradora}</td>
    <td style="text-align:right">${fmtMoney(cm.comisionVendedor)}</td>
    <td style="text-align:center"><input type="checkbox" class="liq-iva-check" data-monto="${cm.comisionVendedor}" onchange="recalcularLiquidacion()" title="Aplica IVA a esta comisión"></td>
  </tr>`).join('');
...
function recalcularLiquidacion(){
  ...
  // Mostrar input de % si hay alguna fila con IVA o switch global activo
...
async function guardarLiquidacion(){
  ...
  const checks=document.querySelectorAll('.liq-com-check:checked');
  if(!checks.length){err.textContent='Selecciona al menos una comisión';err.style.display='block';return;}
  const ivaPct=parseFloat(document.getElementById('liq-iva-pct').value)||15;
  let subtotal=0; const comisionIds=[]; const detalle=[];
  checks.forEach(ch=>{
    subtotal+=parseFloat(ch.dataset.monto)||0; comisionIds.push(ch.dataset.id);
    const cm=comisiones.find(x=>x.id===ch.dataset.id);
    if(cm)detalle.push({inicio:cm.inicio||'',cliente:cm.cliente,aseguradora:cm.aseguradora,ramo:cm.ramo,primaNeta:cm.primaNeta,comision:cm.comisionVendedor});
  });
  let montoIva=0;
  document.querySelectorAll('.liq-iva-check').forEach(cb=>{
    const fila=cb.closest('tr');
  ...
  const data={vendedor,comisionIds,detalle,subtotal:Math.round(subtotal*100)/100,ivaOn,ivaPct,montoIva:Math.round(montoIva*100)/100,total:Math.round(total*100)/100,descuentosAplicados,totalDescuentos:Math.round(totalDescuentos*100)/100,totalNeto:Math.round((total-totalDescuentos)*100)/100};
  try{const nv=await apiPost('liquidaciones',data);liquidaciones.unshift(nv);await cargarComisiones();cerrarModal('overlay-liquidacion');renderComisiones();mostrarToast('Liquidación generada');}
  catch(e){err.textContent='Error al generar';err.style.display='block';}
}
async function eliminarLiquidacion(id){if(!confirm('¿Eliminar esta liquidación?'))return;try{await apiDel('liquidaciones',id);liquidaciones=liquidaciones.filter(x=>x.id!==id);await cargarComisiones();renderComisiones();mostrarToast('Liquidación eliminada');}catch(e){mostrarToast('Error al eliminar');}}
...
function cargarComisionesParaCobrar(){
  ...
  const pendientes=comisiones.filter(cm=>cm.aseguradora===aseg&&!cm.facturado);
  if(!pendientes.length){tbody.innerHTML='<tr><td colspan="9"><div class="empty-state" style="padding:1rem">No hay comisiones pendientes de facturar.</div></td></tr>';return;}
  const ivaOn=document.getElementById('liqcob-iva-switch').checked;
  tbody.innerHTML=pendientes.map(cm=>`<tr>
    <td><input type="checkbox" class="liqcob-com-check" data-id="${cm.id}" data-monto="${cm.comisionBroker}" checked onchange="recalcularLiqCobro()"></td>
    <td>${fd(cm.inicio||cm.fechaCreacion)}</td>
    <td>${cm.cliente}</td>
    <td>${cm.ramo}</td>
    <td>${cm.numero||'N/A'}</td>
    <td style="text-align:right">${fmtMoney(cm.primaNeta)}</td>
    <td style="text-align:right">${cm.pctAseguradora}%</td>
    <td style="text-align:right;font-weight:600">${fmtMoney(cm.comisionBroker)}</td>
    <td style="text-align:center"><input type="checkbox" class="liqcob-iva-check" data-monto="${cm.comisionBroker}" ${ivaOn?'checked':''} onchange="recalcularLiqCobro()" title="Aplica IVA a esta póliza"></td>
  </tr>`).join('');
  recalcularLiqCobro();
}
...
function recalcularLiqCobro(){
  ...
  const ivaPct=parseFloat(document.getElementById('liqcob-iva-pct').value)||15;
  // Cualquier fila con IVA marcado activa el cálculo, independientemente del switch global
  const hayIvaFila=document.querySelectorAll('.liqcob-iva-check:checked').length>0;
  const ivaActivo=ivaGlobal||hayIvaFila;
  // Mostrar/ocultar input de porcentaje si hay algún IVA activo
  document.getElementById('liqcob-iva-row').style.display=ivaActivo?'flex':'none';
  // Calcular IVA: sobre filas marcadas con checkbox individual Y que estén seleccionadas
  let montoIva=0;
  document.querySelectorAll('.liqcob-iva-check:checked').forEach(cb=>{
    const row=cb.closest('tr');
    const comCheck=row?row.querySelector('.liqcob-com-check'):null;
...
async function guardarLiqCobro(){
  ...
  checks.forEach(ch=>{
    subtotal+=parseFloat(ch.dataset.monto)||0; comisionIds.push(ch.dataset.id);
    const cm=comisiones.find(x=>x.id===ch.dataset.id);
    if(cm){if(cm.polizaId)polizaIds.push(cm.polizaId);detalle.push({inicio:cm.inicio||'',cliente:cm.cliente,ramo:cm.ramo,numero:cm.numero||'',primaNeta:cm.primaNeta,pct:cm.pctAseguradora,comision:cm.comisionBroker});}
  });
  // Calcular IVA por fila (solo las marcadas con checkbox individual de IVA)
  let montoIva=0;
  if(ivaOn){
    document.querySelectorAll('.liqcob-iva-check:checked').forEach(cb=>{
      const row=cb.closest('tr');
  ...
  const data={aseguradora,comisionIds,polizaIds,detalle,subtotal:Math.round(subtotal*100)/100,ivaOn,ivaPct,montoIva:Math.round(montoIva*100)/100,total:Math.round(total*100)/100,descuentosAplicados:descApl,totalDescuentos:Math.round(totalDesc*100)/100,totalNeto:Math.round((total-totalDesc)*100)/100};
  try{const nv=await apiPost('liquidaciones-cobros',data);liqCobros.unshift(nv);await cargarPolizas();await cargarComisiones();cerrarModal('overlay-liqcobro');renderComisiones();mostrarToast('Liquidación por cobrar generada');}
  catch(e){err.textContent='Error al generar';err.style.display='block';}
}
async function eliminarLiqCobro(id){if(!confirm('¿Eliminar esta liquidación?'))return;try{await apiDel('liquidaciones-cobros',id);liqCobros=liqCobros.filter(x=>x.id!==id);await cargarPolizas();await cargarComisiones();renderComisiones();mostrarToast('Liquidación eliminada');}catch(e){mostrarToast('Error al eliminar');}}
