...
function cargarComisionesParaLiquidar(){
  ...
  if(!pendientes.length){tbody.innerHTML='<tr><td colspan="9"><div class="empty-state" style="padding:1rem">No hay comisiones facturadas pendientes de pago.</div></td></tr>';document.getElementById('liq-resumen').innerHTML='';return;}
...
async function guardarLiquidacion(){
  ...
  checks.forEach(ch=>{
    ...
    if(cm){
      ...
      const p=polizas.find(x=>x.id===cm.polizaId);
      detalle.push({inicio:cm.inicio||'',cliente:cm.cliente,aseguradora:cm.aseguradora,ramo:cm.ramo,tipoContrato:(p&&p.tipoContrato)||'nuevo',primaNeta:cm.primaNeta,pctVendedor:pctVendUsado,comision:montoUsado});
    }
  });
  let montoIva=0;
  document.querySelectorAll('.liq-iva-check').forEach(cb=>{
...
function cargarComisionesParaCobrar(){
  ...
  if(!pendientes.length){tbody.innerHTML='<tr><td colspan="10"><div class="empty-state" style="padding:1rem">No hay comisiones pendientes de facturar.</div></td></tr>';return;}
...
async function guardarLiqCobro(){
  ...
  checks.forEach(ch=>{
    ...
    if(cm){
      ...
      // Recuperar el % editado desde el input de la fila
      const row=ch.closest('tr');
      ...
      const p=polizas.find(x=>x.id===cm.polizaId);
      detalle.push({inicio:cm.inicio||'',cliente:cm.cliente,ramo:cm.ramo,tipoContrato:(p&&p.tipoContrato)||'nuevo',numero:cm.numero||'',primaNeta:cm.primaNeta,pct:pctUsado,comision:montoUsado});
    }
  });
