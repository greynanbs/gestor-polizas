...
function cargarComisionesParaLiquidar(){
  ...
  const pendientes=comisiones.filter(cm=>{
    ...
    // Póliza cobro mensual + pago anual: aparece cuando al menos una cuota de cobro esté liquidada
    if(p.formaCobroAseg==='mensual'){
      const alMenosUnaCuotaLiquidada=(cuotasMensuales||[]).some(c=>
        c.polizaId===p.id && c.tipoCuota==='cobro' && c.estado==='liquidada'
      );
      if(!alMenosUnaCuotaLiquidada)return false;
    } else {
      // Póliza cobro anual: exige que la comisión anual haya sido facturada a la aseguradora
      if(!cm.facturado)return false;
    }
    return true;
  });
